...
class Docent extends Gebruiker {
	 ...
	 * Constructor
	 *
	 * @param int|string|stdClass|WP_User $docent_id User's ID, a WP_User object, or a user object from the DB.
	 * @param string                      $name      Optional. User's username.
	 * @param int                         $site_id   Optional Site ID, defaults to current site.
	 * @suppressWarnings(PHPMD.ShortVariable)
	 */
	public function __construct( $docent_id = 0, $name = '', $site_id = null ) {
		parent::__construct( $docent_id, $name, $site_id );
		$result = get_user_meta( $docent_id, self::META_KEY, true ) ?: [];
		if ( is_array( $result ) ) {
