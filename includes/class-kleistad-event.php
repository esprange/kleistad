...
class Kleistad_Event {
	...
	public function __construct( $event ) {
		if ( is_string( $event ) ) {
			try {
				$this->event        = Kleistad_Google::service()->events->get( Kleistad_Google::kalender_id(), $event );
	...
	public function save() {
		...
		if ( is_null( $this->event->getCreated() ) ) {
			$this->event = Kleistad_Google::service()->events->insert( Kleistad_Google::kalender_id(), $this->event );
		} else {
			$this->event = Kleistad_Google::service()->events->update( Kleistad_Google::kalender_id(), $this->event->getId(), $this->event );
		}
	}
	...
	public function delete() {
		Kleistad_Google::service()->events->delete( Kleistad_Google::kalender_id(), $this->event->getId() );
	}
	...
	public static function query( $query = [] ) {
		$default_query = [
			'calendarId'   => Kleistad_Google::kalender_id(),
			'orderBy'      => 'startTime',
			'singleEvents' => true,
			'timeMin'      => date( 'c', mktime( 0, 0, 0, 1, 1, 2018 ) ),
			// phpcs:ignore 'privateExtendedProperty' => 'key=' . self::META_KEY,
		];
		$results = Kleistad_Google::service()->events->listEvents( Kleistad_Google::kalender_id(), array_merge( $default_query, $query ) );
